...
const Snipz = async () => {
  ...
  const getAllSnippets = async () => {
    ...
    const snipsWithUsers = snippets.map((snip) => ({ ...snip, user: getUser(snip.userId) }));
      ...
      {allSnippets.map((snip) => (
        <div className="p-3 rounded-md shadow-lg my-10 bg-black bg-opacity-20 backdrop-blur-sm">
          <div className="flex justify-between items-center mb-2">
            ...
            <img src={snip.user.image} alt="user" className="w-[30px] h-[30px] rounded-full shadow-lg" />
            </Link>
          </div>
          <p className="mt-1 max-w-[400px] p-3 bg-slate-900 rounded-sm shadow-lg">{snip.summary}</p>
          <Snippet text={snip.code} language={snip.language} user={snip.user.name.split(" ").join("")} />
        </div>
      ))}
    </div>
  );
